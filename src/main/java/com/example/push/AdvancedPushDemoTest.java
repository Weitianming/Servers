...
/**
 * 推送的OpenAPI文档
 * https://help.aliyun.com/document_detail/mobilepush/api-reference/openapi.html
 */
public class AdvancedPushDemoTest extends BaseTest {
	 ...
	 * <p>
	 * 参见文档
	 * https://help.aliyun.com/document_detail/mobilepush/api-reference/push
	 * -advanced.html
	 */
	@Test
	public void testPushNoticeToAllDeviceTypeAndAllDevice() throws Exception {
		...
		pushRequest.setiOSExtParameters("{\"k1\":\"ios\",\"k2\":\"v2\"}"); // 自定义的kv结构,开发者扩展用
																			// 针对iOS设备
		pushRequest.setApnsEnv("DEV");
		pushRequest.setRemind(true); // 当APP不在线时候，是否通过通知提醒
		// 推送配置: Android
		pushRequest.setAndroidOpenType("3"); // 点击通知后动作,1:打开应用 2: 打开应用Activity
												// 3:打开 url
		pushRequest.setAndroidOpenUrl("http://www.baidu.com"); // Android收到推送后打开对应的url,仅仅当androidOpenType=3有效
		pushRequest
				.setAndroidExtParameters("{\"k1\":\"android\",\"k2\":\"v2\"}"); // 设定android类型设备通知的扩展属性

		// 推送控制
		final Date pushDate = new Date(System.currentTimeMillis() + 30 * 1000); // 30秒之间的时间点,
																				// 也可以设置成你指定固定时间
		...
		final String expireTime = ParameterHelper.getISO8601Time(new Date(
				System.currentTimeMillis() + 12 * 3600 * 1000)); // 12小时后消息失效,
																	// 不会再发送
	...
	public void testPushNoticeToDeviceList() throws Exception {
		...
		// 推送目标
		pushRequest.setAppKey(appKey);
		pushRequest.setTarget("device"); // 推送目标: device:推送给设备;
											// account:推送给指定帐号,tag:推送给自定义标签;
											// all: 推送给全部
		pushRequest.setTargetValue("_YOUR_DEVICE_IDS_HERE"); // 根据Target来设定，如Target=device,
																// 则对应的值为
																// 设备id1,设备id2.
																// 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
		pushRequest.setDeviceType(3); // 设备类型deviceType 取值范围为:0~3. iOS设备: 0;
	 ...
	 */
	@Test
	public void testPushNoticeToAllDeviceTypeAndDeviceWithAccount()
			throws Exception {
		final SimpleDateFormat dateFormat = new SimpleDateFormat(
				"MM-dd HH:mm:ss");
		PushRequest pushRequest = new PushRequest();
		...
		pushRequest.setDeviceType(3); // 设备类型deviceType 取值范围为:0~3. iOS设备: 0;
										// Android设备: 1; 全部: 3, 这是默认值.
		pushRequest.setTarget("account"); // 推送目标: device:推送给设备;
											// account:推送给指定帐号,tag:推送给自定义标签;
											// all: 推送给全部
		pushRequest.setTargetValue("_YOUR_ACCOUNTS_HERE_"); // 根据Target来设定，如Target=device,
															// 则对应的值为
															// 设备id1,设备id2.
															// 多个值使用逗号分隔.(帐号与设备有一次最多1000个的限制)
